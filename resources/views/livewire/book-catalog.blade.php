<div class="flex-1 px-2 py-3">
    ...
    <div class="mb-6">
        <div class="relative">
            <input type="text" wire:model.live.debounce.800ms="search" placeholder="Cari buku..."
            ...
            @if($search)
                <button wire:click="clearSearch" class="absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600">
        ...
        @if($search)
            <div class="mt-2">
                <span class="text-sm text-gray-600">Menampilkan hasil pencarian untuk: "<strong>{{ $search }}</strong>"</span>
            </div>
        @endif
    </div>

    <!-- Books Grid -->
    <div class="w-fit mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 md:grid-cols-2 justify-items-cente gap-y-5 gap-x-7 mt-10 mb-5">
        @forelse($books as $book)
            <!-- Book Card: Image rounded inside the card -->
            <div class="bg-white h-[29rem] w-sm max-w-[14rem] cursor-pointer rounded-xl shadow-md overflow-hidden flex flex-col" wire:key="book-{{ $book->id }}">
            ...
            <div class="col-span-full text-center py-12">
                @if($search)
                    <p class="text-gray-500 text-lg mb-4">Tidak ada buku yang ditemukan untuk pencarian "{{ $search }}"</p>
                    <button wire:click="clearSearch" class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-yellow-500 transition-colors">
                        Hapus Pencarian
                    </button>
                @else
